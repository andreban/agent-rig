...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let stream = runner.run(&agent, vec![Message::user("upload /tmp/report.pdf")].into());
    ...
    let stream = runner.run_with_cancellation(
        &agent,
        vec![Message::user("upload /var/log/app.log")].into(),
        cancel.clone(),
    );
    ...
    let stream = runner.run_with_cancellation(
        &agent,
        vec![Message::user("upload /etc/hosts")].into(),
        cancel.clone(),
    );
