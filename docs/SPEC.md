...
The library is a single crate (`agent-rig`). Provider adapters live in `src/models/`. Agent logic lives in `src/agent.rs` and `src/runner.rs`. The `LlmModel` trait in `src/model.rs` is the extension point. Tool types live under `src/tools/`. The authorization hook lives in `src/auth.rs`.
...
The runner streams: `AgentRunner::run` spawns the agentic loop on a background tokio task and returns a `Stream<Item = AgentEvent>`. Provider errors are surfaced as `AgentEvent::Error(Error)` and terminate the stream; the stream item type is bare `AgentEvent`, not `Result`.
...
The public event type yielded by [`AgentRunner::run`]. Tool call lifecycle events are generated by the runner; `ThinkingDelta` and `TextDelta` are forwarded from the model. Concatenating every `TextDelta` reconstructs the model's final reply.

### `ToolCallResult` (`src/runner.rs`)
...
`run` is the single entry point. It takes the agent and the message thread by value and returns a `Stream<Item = AgentEvent>`. The agentic loop runs on a background tokio task; events are delivered through an internal mpsc channel as they happen. The stream ends after the model produces a turn with no tool calls, or after an `AgentEvent::Error`.
...
Registered via `ToolRegistry::register_agent`.

**How `call` works:**

1. Serializes `args` to a JSON string and passes it as the user message of a fresh run on the inner `AgentRunner`.
2. Returns the child's event stream. The *parent* runner consumes the stream: it forwards every event through the parent stream, and accumulates `TextDelta` chunks into a string that becomes the tool result (wrapped as `Value::String`).
...
- `AgentTool` lives in its own module (`src/tools/agent_tool.rs`) to avoid a circular dependency: `tool.rs` must not import `runner.rs`, and `runner.rs` must not import `agent_tool.rs` directly (it imports it via `crate::tools`).
...
src/
  ...
  runner.rs        — AgentRunner, AgentEvent, ToolCallResult
...
- **Unit tests** live in `#[cfg(test)]` modules inside each source file. Provider calls are replaced with stub/echo `LlmModel` implementations (see `ScriptedModel` in `src/runner.rs`).
- **Integration tests** in `tests/` hit real provider endpoints. They require environment variables (`GEMINI_API_KEY`, running Ollama server) and short-circuit to a no-op return when the required environment is unavailable.
...
3. **Sub-agent dispatch metadata.** A `RunnerEvent { thread_id, depth, agent_event }` envelope is sketched in `src/runner.rs` for nested-agent observability but is not yet wired through `AgentRunner::run`. See `docs/PLAN-subagents-mpsc.md`.
