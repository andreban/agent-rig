...
`Agent` is a pure data blueprint (name, instructions, optional output schema, optional tool name list) with no model reference. `AgentRunner` owns an `Arc<dyn LlmModel>` (cheap to clone) and a list of tool definitions. The runner is cheap to `Clone` so a single runner can be shared across tasks; the same runner can execute multiple agents, and the same agent can be run by different runners backed by different models.

The runner streams: `AgentRunner::run` spawns the agentic loop on a background tokio task and returns a `Stream<Item = RunEvent>`. A `RunEvent` wraps an `AgentEvent` together with the `run_id` of the run that produced it. Provider errors are surfaced as `AgentEvent::Error(Error)` and terminate the stream; the stream item type is bare `RunEvent`, not `Result`.
...
```rust
pub enum AgentEvent {
    /// Emitted when the model requests a tool call. Carries a `ToolCallRequest`
    /// bundling the call `details` (an `Arc<ToolCall>`), a cancel token, and a
    /// oneshot resolver. The consumer looks up the tool, runs it, and calls
    /// `ToolCallRequest::resolve(result)` to resume the runner; the runner
    /// blocks on that resolution. Hallucinated tool calls (no matching registry
    /// entry) are still emitted — the consumer resolves them with an error so
    /// the assistant turn and tool-result messages stay paired. Within one turn
    /// the runner emits one `ToolCall` per call and runs them concurrently, so
    /// events from parallel calls may interleave; use `ToolCall::id` to correlate.
...
`AgentEvent` is the union of things the runner reports as it drives the agentic loop. `ToolCall` is generated by the runner and resolved by the consumer; `ThinkingDelta` and `TextDelta` are forwarded from the model. Concatenating every `TextDelta` reconstructs the model's final reply.
...
pub enum ToolCallResult {
    ...
    Denied,                 // consumer denied its own approval prompt
    Unknown,                // model called a tool not registered in the registry
}
```

A convenience enum the consumer can build to describe a call's outcome and hand to `ToolCallRequest::resolve` — `From<ToolCallResult> for Value` envelopes each variant into the JSON the model receives (`Ok` passes the value through; `Err`, `Denied`, and `Unknown` become descriptive strings). `Denied` covers a call the consumer chose not to run after its own approval prompt; `Unknown` covers a hallucinated tool name, where resolving with *some* result still keeps the assistant turn and tool-result messages paired. Using `ToolCallResult` is optional — the consumer may resolve with any `serde_json::Value` directly.
...
// `Box<dyn Tool>`.
//
// A call is a single phase: `call` receives the whole `ToolCall`, runs the
// tool, and returns its result.
#[async_trait]
pub trait Tool: Send + Sync {
    ...
    /// `tool_call` carries the model-supplied `id`, `name`, and `args`;
    /// deserialize `tool_call.args` into the tool's expected parameters.
    async fn call(
        &self,
        tool_call: std::sync::Arc<crate::model::ToolCall>,
...
`Tool` is the object-safe trait the registry stores: its arguments and results are raw `serde_json::Value`s. A call is a single phase: `call` receives the whole `ToolCall` (its `id`, `name`, and `args`), runs the tool, and returns the result. A tool deserializes `tool_call.args` into whatever parameters it expects. Gating a call behind user approval is the consumer's concern, applied in the event loop before `call` is invoked (see [Client-Side Authorization Patterns](#client-side-authorization-patterns)); a tool that reaches `call` may assume it is authorized to run.

`call` returns a `ToolResult` — `Ok(value)` on success, `Err(value)` for a failure the model should see. Tool errors are *soft*: an `Err` is not a control-flow failure that aborts the run; converting the `ToolResult` into the JSON the model receives envelopes the payload as `{"success": …}` or `{"error": …}` so the model can tell them apart and react.
...
Owns the LLM model and a list of tool definitions. The runner is `Clone` (internals are `Arc`) so it can be shared across tasks. The same runner can execute multiple agents; the same agent can be run by different runners backed by different models.
...
**Cancellation checkpoints.** The runner races against the cancel token at these points: (a) the top of the loop, before building the next `ModelRequest`; (b) `model_stream.next()` (a `tokio::select!` against `cancel.cancelled()` — winning the cancel arm drops the model stream, aborting the in-flight reqwest); and (c) during the wait for each tool call's oneshot resolver to fire. On cancellation the runner emits a single terminal `AgentEvent::Cancelled` and exits. The `CancellationToken` is packed inside `ToolCallRequest`, allowing the client's tool execution to cooperatively select on it and abort cleanly.
...
Because tool execution has been inverted to a client-resolved model, the runner no longer executes tools or handles approval requests internally. Instead, when a tool is called, the runner emits an `AgentEvent::ToolCall(call)` event and blocks until the client resolves it.

Gating tool calls behind user approval is orchestrated entirely on the client side. Whether a call needs confirmation is the consumer's policy — there is no approval hook on the `Tool` trait. The consumer applies its policy (by tool name, argument inspection, a config allowlist, or any other rule) before invoking `Tool::call`:

- **`Tool::call(&self, tool_call: Arc<ToolCall>, cancel: CancellationToken) -> ToolResult`**
   - Executes the tool call once the consumer has authorized it (or immediately, when no approval is required), and returns the result for the model.

#### Example Client Authorization Flow

The client orchestrates approval and execution in the event loop:

```rust
// The consumer's own policy: which tools to gate behind a prompt.
...
while let Some(event) = stream.next().await {
    match event.agent_event {
        AgentEvent::ToolCall(tool_call) => {
            ...
            // 1. Apply the approval policy and prompt the user if needed
            if TOOLS_NEEDING_APPROVAL.contains(&tool_call.details.name.as_str()) {
                let approved = prompt_user(
                    &tool_call.details.name,
                    &tool_call.details.args,
                ).await;
                if !approved {
                    tool_call.resolve(ToolResult::error(
                        "User rejected approval of this tool call.",
                    ));
                    continue;
                }
            }

            // 2. Run the (authorized) tool and resolve the runner
...
This design keeps the runner simple and stateless, giving the consumer absolute control over user prompting, concurrent approval requests, and execution side-effects. For a complete working CLI implementation, see [`examples/mpsc_auth_flow.rs`](examples/mpsc_auth_flow.rs).
...
Provider adapters are opt-in via Cargo features. The core types (`LlmModel`, `Agent`, `AgentRunner`, `Tool`, `ToolRegistry`, `Error`, etc.) are always available regardless of which features are enabled.
